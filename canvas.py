...
class GraphCanvas(QWidget):
    ...
    def getNodeAtPosition(self, x, y):
        ...
        for node in self.graph_data["nodes"]:
            ...
            dx = x - node["x"]
            dy = y - node["y"]
            if dx*dx + dy*dy <= 225:  # 10*10 节点半径为10
                return node
        return None
    ...
    def mousePressEvent(self, event):
        if self.addNodeMode:
            ...
            self.update()
        elif self.addEdgeMode:  # 使用属性而不是通过hasattr检查
            # 处理添加边模式下的点击
            node = self.getNodeAtPosition(event.x(), event.y())
            if node:
                print(f"选中节点: {node['id']}")  # 调试信息
                ...
                if len(self.selectedNodes) == 1:
                    ...
                    print(f"添加边: {start_node['id']} -> {end_node['id']}, directed={self.edgeDirected}")  # 调试信息

                    # 计算两点之间的欧氏距离作为权重
                    dx = (start_node["x"] - end_node["x"]) / 50
                    dy = (start_node["y"] - end_node["y"]) / 50
                    weight = round(math.sqrt(dx * dx + dy * dy), 2)
                    ...
                    # 打印调试信息
                    print(f"添加边成功: {edge_info}")

                    # 重置状态
                    self.selectedNodes = []
                    if hasattr(self.parent(), 'statusBar'):
                        self.parent().statusBar().showMessage(f"已添加边: {start_node['id']} -> {end_node['id']}")
    ...
    def paintEvent(self, event):
        ...
        for node in self.graph_data["nodes"]:
            ...
            if self.searchOrder and (node["id"] == self.searchOrder[0] or (
                    node["id"] in self.searchOrder and node["id"] == self.end_id)):
            ...
            if self.showNodeIDs:
                ...
                painter.drawText(node["x"] - 7, node["y"] + 5, node["id"])
        ...
        for edge in self.graph_data["edges"]:
            ...
            # 如果是有向边
                if edge.get("directed", False):
                    # 画圆弧端点（与无向边一致）
                    start_x = x1 + round(15 * math.cos(theta1))
                    start_y = y1 + round(15 * math.sin(theta1))
                    end_x = x2 + round(15 * math.cos(theta2))
                    end_y = y2 + round(15 * math.sin(theta2))
                    ...
                    # 创建并填充箭头三角形，箭头尖端指向圆弧端点
                    arrow_path = QPainterPath()
                    arrow_path.moveTo(end_x, end_y)  # 箭头尖端在圆弧端点
                    ...
                    # 无向边直接画线
                    painter.drawLine(x1 + round(15*math.cos(theta1)), y1 + round(15*math.sin(theta1)), x2 + round(15*math.cos(theta2)), y2 + round(15*math.sin(theta2)))

                # 在绘制权值时，计算边的法线方向，并将文本沿法线偏移
